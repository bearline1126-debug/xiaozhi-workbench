...
- **v99（2026-08-22）例句翻译 + 排版三行制**：搜索结果改为三行（词条 / 中文释义 / 例句+译文）；大词库词条命中后自动让 AI 补一条例句并附中文翻译（缓存进 `english_ai_dict` 的 ex/exzh，失败存空串防重复请求）；例句「译」按钮 + `english_ex_zh` 缓存（上限 600，按例句文本去重）——每日单词/固定搭配卡片的例句同样可点「译」；搜索按钮改为无边框纯 🔍 图标。
- v98（2026-08-22）：新增离线大词库 `dict.json`（ECDICT 筛选 5.9 万词条/3.2MB，搜索第三层，SW 预缓存）；搜索链四层：种子池→词形→大词库→AI。
- v97（2026-08-22）：搜索 UI 两行制、✚ 图标、AI 查词带例句、「来源」简化。

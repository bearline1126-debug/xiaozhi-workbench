...
- **v100（2026-08-22）例句翻译全自动 + 删听力模块**：每日单词/固定搭配卡片的例句翻译改为自动出（进页面后台翻一次并缓存 `english_ex_zh`，直接显示译文、无「译」按钮无前缀、静默失败）；搜索结果的例句译文同样去掉前缀；删除「🎧 听力资源/播客」卡片及其 CSS（用户反馈点不开且慢）。
- v99（2026-08-22）：搜索结果三行制；大词库命中自动 AI 补例句带中文翻译；例句译缓存机制。
- v98（2026-08-22）：新增离线大词库 `dict.json`（ECDICT 筛选 5.9 万词条/3.2MB，SW 预缓存）；搜索链四层：种子池→词形→大词库→AI。

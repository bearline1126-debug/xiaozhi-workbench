...
- **v138（2026-09-10 经验摘录编辑弹框·导入/备份提示·存储说明统一修复）**：修用户四点反馈。①**✏️ 编辑点无反应（要退出详情页才弹）**：根因 `.ui-modal-mask` z-index(9998) 低于全屏详情层 `.ui-fullscreen`(9999) 被盖住 → 改到 10000，点编辑立即弹框；编辑弹框的**标签输入框加 ▾ 下拉**（复用 `Util.Combo.open('niTags','note_tags')` 选已用标签，与主表单一致）。②**导入去掉全屏「正在导入」大页**：改底部 toast「⏳ 正在导入，请稍候…」（`UI.toast(msg, keep)` 持续显示直到被替换），成功变「✅ 导入成功（含 N 个媒体）」再延迟 1200ms 刷新；失败 `alert` 透出真实原因再刷新——不再出现「导入半天无提示直接回首页」。③**备份成功无提示**：给 `UI.toast` 加 `keep` 参数（不自动消失，直到被下一条替换）；原生备份「⏳ 正在备份…」改为持续显示、成功后替换成「✅ 备份成功，已写入『…』」，网页分支也补了「正在备份」进度，两端行为统一（成功前进度一直可见，成功即变成功提示）。④**存储空间说明**：把「27 个文件」措辞改「N 张图片/视频」，并在存储卡加一行说明——「图片/视频」是添加的原图独立存放，导出/备份会把原图转成 base64 打包进一份 JSON，所以备份比这里显示的更大属正常。语法校验通过；版本三处同步 `2026-09-10-v138`；APK www 三件套已从网页版全量同步。

- **v137（2026-09-10 按需移除「清理残留」+ 吐槽区圆点 + 经验摘录弹窗改信息）**：按用户拍板逐项做。①**移除「清理残留」功能**（用户上次被 v135 误删过真实图片，明确不想再用该功能）——删掉设置卡「存储空间」里的按钮与说明、删掉 `Media.cleanOrphans()` 整个方法、给存储将满的 `storeWarn` 提示去掉了「② 再点清理残留」的建议（改为建议先备份、必要时整理删除不用的记录）。这样彻底没误删入口，副作用是删卡片遗留的孤儿图会一直占空间，但不影响使用。②**日记吐槽区每行加圆点**：`DiaryBook` 摘要渲染吐槽区 `<ul class="db-list">` 加 `dots` class，新增 `.db-box.cyan .db-list.dots { list-style:disc; padding-left:18px }`（吐槽区 cyan 专用，不影响其他手账色块），每行吐吐槽现在有小圆点标记起点，和列表一样一眼看出几条。③**经验摘录详情页右上角加 ✏️ 编辑**：`Notes.openDetail` 的 `rightBtn` 最前面加编辑符号按钮（调用新方法 `Notes.editInfo(id)`），点它弹出 `UI.modal('编辑摘录信息')` 可改**题目 / 分类（下拉，含现有分类）/ 标签（逗号分隔）**三条，保存即改 `Store('notes')` 里对应记录（title 留空时自动回填正文第一句）、刷新列表并 toast「摘录信息已更新」。④同步 APK：`www/index.html`+`sw.js`+`manifest.json` 已从网页版全量拷贝（与主文件字节一致、确认 v137、无 cleanOrphans、有 editInfo 与 dots）。语法校验通过；版本三处同步 `2026-09-10-v137`。

- **v136（2026-09-09 备份「写入失败」误报根因 + 进度条用手机真实内存 + 清理残留误删大 bug 修复）**：真机截图 to
ast「写入失败：临时文件缺失，无法转正」定位到真根因。①**修复「临时文件缺失，无法转正」**：原生 `BackupSavePlugin.promote()` 原先用 `df.findFile(fileName + ".part")` 按名字找临时文件，但 provider（DocumentsProvider）可能规范化/改写落盘后的展示名导致找不到 → 明明写完了却报此错（这就是「备份成功却提示失败」的真根因，v135 只修了转正后收尾、没修到点）。v136 改为：openWrite 时把**真正落盘的 .part DocumentFile 存进实例字段 `pendingPart`**，最后一刻 `promote(dir, fileName, part)` 直接用真实引用转正，不再按名字 findFile。②**进度条上限改用本机实际内存**：原生新增 `getStorage()`（`StatFs` 返回设备总/可用存储），JS `renderStorage` 在 APK 下用 `totalBytes` 作上限、明细追加「本机可用 X」，网页兜底用浏览器配额；去掉 v135 的固定 2GB。③**备份/导入加进度反馈**：`runNow` 原生分支开始「⏳ 正在备份，请稍候…」、成功「✅ 备份成功」；`importFile` 导入开始用 `UI.openFull` 弹「⏳ 正在导入…」持久提示卡、结束「导入成功」，读失败也 `location.reload()` 避免卡在加载层。④**修复「清理残留误删真实图片」严重 bug**：`Media.cleanOrphans` 旧实现只把 `visuals/dishes/clothes` 三个模块的 id 当「在用」，漏掉日记、灵感速记、生活、富文本内嵌图(`[img:m_xxx]`/`data-media`/`openFull` 里的 id)，且 dishes 的 `photoId` 已是数组却被当单个 key 匹配——导致把正常图片当孤儿删。v136 改为**全量深度扫描**：遍历 `Store.keyList()` 所有记录，递归收集一切形如 `m_*` 的 id + 从富文本字符串里正则抽取内嵌媒体 id，只把「完全无引用」的当孤儿；且删除前 `confirm` 明确显示将删数量并重申「只删已不被任何卡片/日记/灵感引用的旧图片」。⑤**清理残留说明精简独立一行**：按钮独占一行，下方一句，不再和按钮并排。⑥统一「恢复→导入」文案。语法校验通过；版本三处同步 `2026-09-09-v136`；release APK 已重打**放回 `keystore-release/拾光手账-v136-release.apk`**（解包验：v136、sw cache v136、`INLINE_RE` 全量扫描在、`dict.json` 3MB 英语词典在、dex 含 `getStorage`+`StatFs` 原生新方法）。

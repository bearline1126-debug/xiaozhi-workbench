...
- **v98（2026-08-22）离线大词库**：新增 `dict.json`（ECDICT 筛选版约 5.9 万词条 / 3.2MB：常用 + 中高考/四六级/考研/托福/GRE 标签词 + BNC/COCA 词频前 5 万，仓库外一次性生成勿手改）。英语搜索改为四层链：5000 学术种子池 → 词形归并 → **大词库（离线秒查，SW 预缓存 + 页面进入时后台预热）** → AI 自动查。附带：🔍 搜索按钮再缩小；修 v96 AI 查词旧缓存无例句字段不重查的 bug（无 ex 的缓存视为未命中，重查后覆盖）。
- v97（2026-08-22）：搜索 UI 打磨——两行制结果（词条+释义 / 例句）、✚ 图标加入按钮、AI 查词返回例句、「来源」简化。
- v96（2026-08-22）：matchNotes 恢复 2-gram 模糊匹配（三层防噪）；词典自动 AI 查词 + `english_ai_dict` 缓存。

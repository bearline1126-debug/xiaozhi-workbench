...
- **v94（2026-08-22）三功能**：① 闲话铺 AI 去死板——temperature 0.3→0.8、删「第一句必须是问句」的固定开场方向改为「换着来」、打招呼只在每天第一条（续聊不再打招呼）；② 首页「今日必做」卡片加 ✏️ 按钮——弹窗内增删改/排序/关联主线任务，保存写 `success:<今天>.todayTasks`（补昨晚忘规划的场景）；③ 英语积累加搜索单词/搭配（输英文或中文释义互查）——结果可朗读、一键「加入今日测试」（存 `english_extra_test:<日期>`，不参与金币判定防刷），答完（对错都行）按 `sw:<词面>` key 建 SRS 条目，明天进复习循环，规则与每日单词一致（3/7/14 天、连对 4 次入已学库）。

...
- **v95（2026-08-22）三修复**：① 修必做编辑器折两行——flex 一行布局的 CSS 挂在 `.must-editor` 类下，v92 的 `dlTodayMustBox` 和 v94 弹窗 `homeMustBox` 都没带该类导致折行，`MustEditor.mount` 现在自动补类（顺带确认：闲话铺的经验摘录引用链路 matchNotes→notePart→refs 一直完整保留，未动）；② 英语搜索加词形归并兜底（searches→search、grasping→grasp 这类变形也能搜到）；③ 词库没有时（内置 5000 条是学术词池非全量词典，如 explicitly 就真没有）给「🤖 让 AI 查」按钮——中英互查，结果同样一键加入今日测试。
- v94（2026-08-22）三功能：① 闲话铺 AI 去死板——temperature 0.3→0.8、放开固定开场方向、打招呼只在每天第一条；② 首页「今日必做」卡片加 ✏️ 补填编辑弹窗（存 `success:<今天>.todayTasks`）；③ 英语积累搜索单词/搭配（中英互查、朗读、一键入今日测试，`english_extra_test:<日期>` + `sw:<词面>` SRS key 进复习循环，不参与金币判定）。

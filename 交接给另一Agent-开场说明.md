...
- **v96（2026-08-22）两根因修复**：① 闲话铺摘录引用「感觉失效」的根因是 v83 为治引用太滥把 2-gram 模糊匹配一刀切删了（只剩「消息逐字含关键词」一条路，日常口语几乎永不触发）——现恢复 2-gram 匹配并带三层防噪（分词块内滑窗不跨标点、高频虚词 gram 过滤、gram 贡献封顶 9 分），阈值 3（关键词 +8/标签 +5/整块 +3/gram +3 每个封顶 3 个）；② 英语词典改「全量词典体验」——词库（5000 学术词池）没有时自动调 AI 查（不再需要点按钮），结果缓存 `english_ai_dict`（上限 200 条），AI 结果同样可入今日测试。
- v95（2026-08-22）：MustEditor.mount 自动补 `.must-editor` 类（修编辑器折两行）；英语搜索词形归并兜底 + 手动 AI 查词按钮。
- v94（2026-08-22）：闲话铺去死板（temp 0.8 + 放开开场 + 只首条打招呼）；首页今日必做 ✏️ 补填弹窗；英语搜索 + 一键入今日测试（`english_extra_test:<日期>` / `sw:<词面>` SRS key）。

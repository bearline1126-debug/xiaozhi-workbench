...
/* 从 IndexedDB 读取用户自定义的 App 名称/图标（供 manifest 动态生成） */
...
async function buildManifest(url){
  ...
  if(custom && (custom.name || custom.shortName || custom.icon)){
    const manifest = {
      ...
      icons: [
        {src: custom.icon || 'icon-192.png', sizes: '192x192', type: 'image/png', purpose: 'any'},
        {src: custom.icon || 'icon.png', sizes: '512x512', type: 'image/png', purpose: 'any'}
      ]
    };
...
self.addEventListener('fetch', event => {
  ...
  /* manifest.json：动态注入用户自定义的 App 名称/图标 */

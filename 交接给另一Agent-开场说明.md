...
- **v128（2026-09-08 首页「生活万花筒」与「今天吃什么」并排）**：默认首页布局里生活万花筒原来占整行（`size:'wide'`），被调成 `size:'small'` 半格后自动和下方的今天吃什么（本就半格）在同一行**并排显示**，两卡平衡更好看；生活万花筒默认展开（`collapsed:false`）显示 2×2 迷你数字。加了一次性迁移 `_homeWheelLifeLayout`：若用户当前 `home_widgets` 仍是旧默认布局（tasks/tracker/life wide、wheel small、entries wide，未自定义过）则自动刷新为新默认，已自定义过的不动。三处版本同步 `2026-09-08-v128`。

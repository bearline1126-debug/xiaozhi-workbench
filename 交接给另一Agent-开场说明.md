...
- **v101（2026-08-22）**：每日对话逐行对齐——一句英文下紧跟对应中文（种子池的中文是挤在一行的，按说话人标记 A：/B：切开配对，实测 500 条全对齐；AI 生成内容按行配对）。
- v100（2026-08-22）：例句翻译全自动（无按钮无前缀、缓存 `english_ex_zh`、静默失败）；删除「听力资源/播客」模块。
- v99（2026-08-22）：搜索结果三行制；大词库命中自动 AI 补例句带翻译；v98 离线大词库 `dict.json`（5.9 万词条）四层搜索链。

...
- **v126（2026-09-08 设置页新增「打赏作者」：自愿收款码打赏）**：按用户"分享为主、顺带赚点小钱，以打赏作者方式"的定位，在**设置页「关于」卡片**加「🎁 打赏作者」按钮 → `Settings.donate()` 用现成 `UI.modal` 弹层展示打赏说明 + 收款码图。**注意：收款码图必须用户自备（AI 无法生成真实可用的收款码）**，存为 `assets/donate-qr.png` 即自动显示；未放时 img onerror 隐藏图、显示"收款码待放好"提示。纯自愿、金额随意。**版本三处已同步 `2026-09-08-v126`，暂未部署**——等用户提供收款码图后再推送，避免上线后弹窗里只是占位提示。

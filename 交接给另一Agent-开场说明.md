...
- **v119（2026-09-02 翻页改为「翻页前保持旧页、翻完才换新页」）**：**修 v118 引入的翻页时序问题**——v118 习惯性在动画开始前就把 `left/right` 换成新对开页，导致"点箭头瞬间整张先变新、翻到一半白纸、完了又显示一遍"。现改为：**翻页动画期间 `left/right` 维持旧内容（翻页前/中保持旧页）**，书叶正面=被翻走那页（旧内容可见）、背面留白；等到 `done()`（transitionend 或 620ms 兜底）才在同一同步块内「先填新对开页 → 再移除书叶」，避免闪旧也避免闪白。满足"翻页前是旧内容、翻过去才是新内容"。三个版本号同步 → `v119`。（注：金币系统本轮已通盘审计，调用链完整、Coin.earn 无断点，未改动金币代码；详见会话向用户说明。）

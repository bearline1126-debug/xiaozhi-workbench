...
- **v102（2026-08-22）**：① 每日对话改回两段式——先整段英文、再整段中文，中文经 `dialogPairs()` 逐行配对后带换行（一行英文对一行中文）；② 每日记录页新增「感恩瞬间（可上传图片）」区块（位于今日必做之下、明天必做之上），与感恩日记页共用 `success:<date>.images`，`Success.renderImages/uploadGratitude/removeImage` 泛化为带 boxId/date 参数（默认值保持旧调用兼容），补记模式跟随所选日期。
- v101（2026-08-22）：每日对话中文按说话人标记切开与英文配对（dialogPairs 方法，后续排版复用）。
- v100（2026-08-22）：例句翻译全自动（缓存 `english_ex_zh`）；删除听力模块。v99/v98：搜索三行制、AI 补例句、离线大词库 `dict.json` 四层搜索链。

...
- **v97（2026-08-22）搜索 UI 打磨**：查按钮改 🔍 图标（搜索框占满剩余宽度）；搜索结果改两行制（第一行 词条+释义，第二行 例句展示用法）；「加入今日测试」改 ✚ 图标按钮；AI 查词 prompt 增加 ex 字段（返回例句，缓存兼容旧格式）；「今日内容来源」简化为「来源」。
- v96（2026-08-22）：matchNotes 恢复 2-gram 模糊匹配（v83 一刀切删掉导致口语几乎永不引用；三层防噪：块内滑窗/虚词过滤/贡献封顶）；词典词库没有时自动调 AI 查（缓存 `english_ai_dict` 上限 200）。
- v95（2026-08-22）：MustEditor.mount 自动补 `.must-editor` 类（修编辑器折两行）；英语搜索词形归并兜底。

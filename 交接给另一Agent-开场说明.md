...
- **v118（2026-09-02 开心小事卡片换淡黄 + 修翻页旧页残影）**：① **日记本「⭐开心小事」卡片由粉色改淡黄色**——`db-box pink` → `db-box yellow`（复用已存在的淡黄样式 `#fdf3c9`/边 `#f0e2a4`/标题 `#8a7a4a`，与整本手账纸张色 `#fffef8` 协调）；② **修翻页「先显示旧一页、约1秒后跳成新内容」**：根因是老代码给翻页书叶的**正、背面都塞了旧页内容**（`f`/`b` 都 `innerHTML=leafHtml`），移动端 double-side 3D 渲染不稳，翻过去后背面旧内容盖着已铺好的新对开页约 0.6s 再突跳（兜底 `setTimeout(done,700)` 也加重了迟滞）。改为**叶子正面=即将翻走的那页、背面留白**，下方新对开页照常铺好——翻页只见纸页翻走、背面空白过渡，翻完立即露出新页（「先空白→直接更新」），无旧内容残影；兜底改 `620ms`。三个版本号同步 → `v118`。
